...
import { ListBullets, ListNumbers, Link as LinkIcon, TextB, TextItalic } from "@phosphor-icons/react"
...
export function TextEditor({
  ...
  return (
    <div className={cn("rounded-md border bg-background text-foreground", className)}>
      {showToolbar && <Toolbar editor={editor} className={toolbarClassName} controls={controls} />}
      <EditorContent
        editor={editor}
        className={cn(
...
function Toolbar({ editor, className, controls }: ToolbarProps) {
  ...
  return (
    <div className={cn("flex items-center gap-1 border-b bg-muted/40 px-2 py-1", className)}>
      {controls?.bold !== false && (
        <Button
          type="button"
          variant="ghost"
