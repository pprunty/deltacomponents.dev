...
const themeMapping: Record<ThemeOption, { light: string; dark: string }> = {
...
// Cache key generator
const getCacheKey = (code: string, language: string, theme: string) =>
...
export function CodeBlock({
  ...
  return (
    <div className={cn(
      "relative w-full",
      border && "rounded-xl overflow-hidden border border-border bg-card text-card-foreground shadow-sm"
    )}>
      ...
      <div className="relative w-full code-block-container">
        ...
        {showCopyButton && mounted && !isLoading && (
          <div className="absolute right-4 top-2 z-10">
            ...
            >
              {copied ? <Check size={20} weight="bold" /> : <ClipboardText size={20} />}
            </button>
          </div>
        )}
<div
  ref={contentRef}
  className={cn(
    "py-4 px-2 w-full code-content",
    !expanded && "max-h-[var(--code-block-max-height)] overflow-y-auto", // Added overflow-y-auto here
  ...
  {renderContent()}
        ...
        {showExpandButton && contentOverflows && mounted && (
          <button
            ...
            className={cn(
              "absolute right-4 p-2.5 rounded-md",
              "bg-background/80 backdrop-blur-sm",
              "text-muted-foreground hover:text-foreground hover:bg-background",
              "transition-all duration-200",
              "shadow-sm hover:shadow-md",
