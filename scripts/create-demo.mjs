...
async function main() {
  try {
    ...
    if (!component) {
      console.error(`Component ${componentName}.tsx does not exist in any registry subdirectory!`);
      process.exit(1);
    }
    
    console.info(`Found component ${componentName}.tsx in registry/${component.category}`);
    ...
    const demoContent = await generateDemoContent(componentName, demoSuffix, componentContent, component.category);
...
async function generateDemoContent(componentName, demoSuffix, componentContent, category) {
...
async function updateComponentDocs(componentName, demoName, category) {
  ...
  if (!docsPath) {
    console.warn(`Warning: Could not find docs file for ${componentName}. Tried paths: ${possiblePaths.join(', ')}`);
    return;
  }
