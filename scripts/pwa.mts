import { promises as fs, existsSync, readdirSync, lstatSync, unlinkSync, copyFileSync, readFileSync, writeFileSync } from 'fs';
...
async function createFaviconIco(svgPath: string): Promise<void> {
  try {
    ...
    await sharp(svgPath).resize({ width: 32, height: 32 }).png().toFile(tempPng);
