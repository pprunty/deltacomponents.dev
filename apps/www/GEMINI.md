...
To create a new theme (e.g., a Gemini-inspired theme named after a constellation):

1. **Create CSS Theme File**: Create `styles/themes/[constellation-name].css`
   ```css
   /* [Constellation Name] (Gemini) Theme */
   html[data-theme="[constellation-name]"] {
     --background: oklch(0.9800 0.0050 240.0000); /* Blue-ish tint */
     --foreground: oklch(0.2500 0.0800 260.0000);
     --card: oklch(0.9850 0.0030 245.0000);
     /* ... all required CSS variables ... */
   }
   
   html[data-theme="[constellation-name]"].dark {
     --background: oklch(0.1100 0.0200 260.0000);
     --foreground: oklch(0.9300 0.0300 240.0000);
...
2. **Import Theme**: Add to `styles/themes.css`
   ```css
   @import './themes/[constellation-name].css';
   ```

3. **Add Theme Data**: Update `lib/theme-data.ts`
   ```typescript
   export const [constellationName]Theme: ThemeData = {
     name: "[Constellation Name] (Gemini)",
     value: "[constellation-name]",
     description: "A Gemini-inspired theme with...",
     previewImage: "/images/themes/[constellation-name].png",
     css: `/* CSS string from the theme file */`
   }
   
   // Add to THEME_DATA array
   export const THEME_DATA = [...existingThemes, [constellationName]Theme]
   ```
...
5. **Add Meta Theme Colors**: Generate hex values and update `lib/config.ts`
   ...
   export const THEME_META_COLORS = {
     // ... existing themes
     [constellationName]: { light: "#fbf7f5", dark: "#03030a" }
   }
   ```
...
1. **Create Demo File**: Place in `registry/delta-ui/examples/`
   ...
   - Example: `card-stack-gemini-demo.tsx`
