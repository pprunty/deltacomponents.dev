...
export const transformers = [
  {
    code(node) {
      if (node.tagName === "code") {
        ...
        // npm exec.
        if (raw.startsWith("npm exec")) {
          node.properties["__npm__"] = raw
          node.properties["__yarn__"] = raw.replace("npm exec", "yarn")
          node.properties["__pnpm__"] = raw.replace("npm exec", "pnpm dlx")
          node.properties["__bun__"] = raw.replace("npm exec", "bunx --bun")
        }
      }
    },
  },
