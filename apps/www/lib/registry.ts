...
function buildPathMappings(
  ...
  files.forEach((file) => {
    if (file.path && file.target) {
      // Extract the source path relative to registry/delta-ui/
      const match = file.path.match(/registry\/delta-ui\/(.+)$/)
      if (match) {
...
export function fixImport(content: string, pathMappings: Map<string, string>) {
  ...
  content = content.replaceAll(
    "@/registry/delta-ui/examples/",
    "@/components/examples/"
  )

  // Fix hook imports
  content = content.replaceAll("@/registry/delta-ui/hooks/", "@/hooks/")

  // Fix lib imports
  content = content.replaceAll("@/registry/delta-ui/lib/", "@/lib/")
