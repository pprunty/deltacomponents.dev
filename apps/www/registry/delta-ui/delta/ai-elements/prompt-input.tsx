...
import {
  ...
  HoverCardContent,
  HoverCardTrigger,
} from "@/registry/delta-ui/ui/hover-card";
import {
  InputGroup,
  InputGroupAddon,
  InputGroupButton,
  InputGroupTextarea,
} from "@/registry/delta-ui/ui/input-group";
import {
  Select,
  SelectContent,
  SelectItem,
  ...
  return (
    <div
      className={cn(
        ...
        mediaType === "image" ? "h-14 w-14" : "h-8 w-auto max-w-full border",
        className
      )}
      key={data.id}
      {...props}
...
export const PromptInputTextarea = ({
  ...
  return (
    <InputGroupTextarea
      className={cn("field-sizing-content max-h-48 min-h-16 text-base", className)}
      name="message"
  ...
  useEffect(() => {
    if (
      ...
      speechRecognition.onerror = (event) => {
        ...
        switch (event.error) {
          case 'not-allowed':
            console.warn('Microphone access denied. Please allow microphone permissions and try again.');
            break;
          case 'no-speech':
            console.warn('No speech detected. Please try speaking again.');
            break;
          case 'audio-capture':
            console.warn('No microphone found. Please check your microphone connection.');
            break;
          case 'network':
            console.warn('Network error occurred during speech recognition.');
            break;
          default:
            console.warn('Speech recognition error occurred:', event.error);
        }
      };
