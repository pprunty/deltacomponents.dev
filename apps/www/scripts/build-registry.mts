...
export const Index: Record<string, any> = {`
  for (const item of registry.items) {
    const resolveFiles = item.files?.map(
      (file) => `registry/delta-ui/${file.path}`
    )
    if (!resolveFiles) {
      continue
    }
    ...
    const componentPath = item.files?.[0]?.path
      ? `@/registry/delta-ui/${item.files[0].path}`
      : ""
  ...
  "${item.name}": {
    ...
    files: [${item.files?.map((file) => {
      const filePath = `registry/delta-ui/${typeof file === "string" ? file : file.path}`
...
async function buildRegistryJsonFile() {
  ...
  await fs.cp(
    ...
    path.join(
      process.cwd(),
      "../www/public/r/styles/delta-ui/registry.json"
    ),
    { recursive: true }
  )
}
  ...
  return new Promise((resolve, reject) => {
    exec(
      ...
      async (error, stdout, stderr) => {
        ...
        // Post-process the generated JSON files to remove registry/delta-ui prefix
    ...
    if (json.files) {
      ...
      json.files.forEach((file: any) => {
        if (typeof file === 'object' && file.path && file.target) {
          ...
          const sourcePath = file.path.replace(/^registry\/delta-ui\//, '')
      ...
      json.files = json.files.map((file: any) => {
        if (typeof file === 'object' && file.path) {
          // Remove registry/delta-ui/ prefix
          let cleanPath = file.path.replace(/^registry\/delta-ui\//, '')
...
function rewriteImports(content: string, pathMappings: Map<string, string>): string {
  ...
  // Rewrite imports from @/registry/delta-ui/lib/... to @/lib/...
  content = content.replace(
    /@\/registry\/delta-ui\/lib\//g,
    '@/lib/'
  )
  ...
  for (const component of uiComponents) {
    ...
    if (component.files) {
      for (const file of component.files) {
        const filePath = `registry/delta-ui/${file.path}`
        ...
        // Clean path - remove registry/delta-ui/ prefix
