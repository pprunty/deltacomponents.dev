...
const DIRECTORIES = argDir ? [argDir] : ["public/posts", "public/projects"]
...
async function processDirectory(dirPath: string): Promise<ConversionResult[]> {
  ...
  for (const file of files) {
    ...
    if (result) {
      ...
      console.log(
        `   Size: ${(result.originalSize / 1024).toFixed(2)}KB → ${(result.webpSize / 1024).toFixed(2)}KB (${result.savedPercent.toFixed(1)}% smaller)\n`
      )
    }
  }
...
async function main() {
  ...
  console.log(
    `💾 Total size before: ${(totalOriginalSize / 1024 / 1024).toFixed(2)} MB`
  )
  console.log(
    `💾 Total size after:  ${(totalWebpSize / 1024 / 1024).toFixed(2)} MB`
  )
  console.log(
    `🎉 Total saved: ${(totalSaved / 1024 / 1024).toFixed(2)} MB (${totalSavedPercent.toFixed(1)}% reduction)\n`
  )
