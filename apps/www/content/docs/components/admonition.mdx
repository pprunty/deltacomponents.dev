...
For long-form technical content or stack traces, use the `expandable` prop. You can control the initial state with `defaultCollapsed`. The entire admonition is clickable when collapsed for better UX.
...
| `expandable`       | `boolean`                                                                      | `false`     | Makes the admonition expandable/collapsible. Entire admonition is clickable when collapsed. |
| `defaultCollapsed` | `boolean`                                                                      | `false`     | Sets the initial state if `expandable` is true.                                             |
| `actions`          | `AdmonitionAction[]`                                                           | —           | Array of action buttons to display at the bottom.                                           |
| `size`             | `"sm" \| "small" \| "default" \| "lg" \| "large"`                              | `"default"` | Controls padding and font sizing.                                                           |
