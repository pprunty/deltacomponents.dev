...
Render multiple tweets in a row using the `xs` size.

<ComponentPreview name="x-card-side-by-side-demo" />
...
// Extra small size (using short form)
<XCard
  id="1825208781827080599"
  size="xs"
  caption="Extra small tweet embed"
/>
...
// Extra large size (using short form)
<XCard
  id="1825208781827080599"
  size="xl"
  caption="Extra large tweet embed"
/>
```
...
| `size`      | `"xs" \| "extra-small" \| "sm" \| "small" \| "default" \| "lg" \| "large" \| "xl" \| "extra-large"` | `"default"` | Controls the display size of the tweet embed. Supports both short (`xs`, `sm`, `lg`, `xl`) and long form aliases  |
...
- **Extra small (`"extra-small"` or `"xs"`)**: 75% scale with `max-w-xs` container width
...
- **Extra large (`"extra-large"` or `"xl"`)**: 125% scale with `max-w-3xl` container width
