...
<Admonition type="info" title="Desktop Only">
  This component only works on desktop devices with a mouse cursor. The effect
  is automatically disabled on mobile and touch devices.
</Admonition>
...
```bash
npx shadcn@latest add https://deltacomponents.dev/r/mouse-string-connection.json
```
