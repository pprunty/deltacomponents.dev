...
<Admonition type="info" title="Desktop Only" className="py-4">
  This component only works on desktop devices with a mouse cursor. The hover
  effect is automatically disabled on mobile and touch devices.
</Admonition>
...
```bash
npx shadcn@latest add https://deltacomponents.dev/r/font-transform.json
```
