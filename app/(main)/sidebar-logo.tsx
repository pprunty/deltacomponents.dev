...
export function SidebarLogo() {
  ...
  const showBackButton = pathname.startsWith('/docs/') || pathname.startsWith('/guide/');
    ...
    <div className="mb-8">
      {showBackButton ? (
        <BackButton 
          ...
          iconProps={{ className: 'w-6 h-6 text-muted-foreground transition-colors group-hover:text-foreground' }}
        />
      ) : (
        <Link href="/">
          <svg
            className="fill-foreground dark:fill-foreground transition-colors hover:opacity-80"
            width="28"
            height="28"
            viewBox="0 0 282 308"
